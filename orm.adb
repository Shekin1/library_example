...
package body Orm is
   ...
   function Hash_Book (Id : Integer) return String;

   function Hash_Customer (Id : Integer) return String;

   function Hash_Dvd (Id : Integer) return String;
   ...
   begin
      return Str_Or_Empty (Book_Data (Self.Get).ORM_Author);
   end Author;
   ...
   begin
      return Str_Or_Empty (Dvd_Data (Self.Get).ORM_Author);
   end Author;
   ...
   begin
      return Book_Data (Self.Get).ORM_Borrowed_By;
   end Borrowed_By;
   ...
   begin
      ...
      if Self.Depth > 0 and then Self.Data.Follow_LJ then
         ...
         return All_Customers.Filter (Id => Self.Borrowed_By)
   ...
   is
      D : constant Book_Data := Book_Data (Self.Get);
      S : Session_Type;
   begin
   ...
   begin
      return Dvd_Data (Self.Get).ORM_Borrowed_By;
   end Borrowed_By;
   ...
   is
      D : constant Dvd_Data := Dvd_Data (Self.Get);
      S : Session_Type;
   begin
      if D.ORM_FK_Borrowed_By = null then
         if not Dynamic_Fetching then
   ...
   begin
      return Str_Or_Empty (Customer_Data (Self.Get).ORM_First);
   end First;
   ...
   begin
      return Customer_Data (Self.Get).ORM_Id;
   end Id;
   ...
   begin
      return Book_Data (Self.Get).ORM_Id;
   end Id;
   ...
   begin
      return Dvd_Data (Self.Get).ORM_Id;
   end Id;
   ...
   begin
      return Str_Or_Empty (Customer_Data (Self.Get).ORM_Last);
   end Last;
   ...
   begin
      return Book_Data (Self.Get).ORM_Pages;
   end Pages;
   ...
   begin
      return Book_Data (Self.Get).ORM_Published;
   end Published;
   ...
   begin
      return Dvd_Data (Self.Get).ORM_Published;
   end Published;
   ...
   begin
      return Dvd_Data (Self.Get).ORM_Region;
   end Region;
   ...
   begin
      return Str_Or_Empty (Book_Data (Self.Get).ORM_Title);
   end Title;
   ...
   begin
      return Str_Or_Empty (Dvd_Data (Self.Get).ORM_Title);
   end Title;
   ...
   begin
      return All_Books.Filter(Borrowed_By => Self.Id);
   end Borrowed_Books;
   ...
   begin
      return All_Dvds.Filter(Borrowed_By => Self.Id);
   end Borrowed_Dvds;
   ...
   begin
      ...
      Free (Tmp.ORM_Author);
      Free (Tmp.ORM_Title);
      Tmp.ORM_Author         := new String'(String_Value (Self, F_Books_Author));
      ...
      Tmp.ORM_Title          := new String'(String_Value (Self, F_Books_Title));
   ...
   is
      ...
      Tmp     : Customer_Data;
   begin
      Tmp := Customer_Data (Result.Get);
      if Tmp = null then
         Tmp := new Customer_DDR;
         Set (Result, Tmp);
      end if;

      Free (Tmp.ORM_First);
      Free (Tmp.ORM_Last);
      Tmp.ORM_First    := new String'(String_Value (Self, F_Customers_First));
      Tmp.ORM_Id       := Integer_Value (Self, F_Customers_Id);
      Tmp.ORM_Last     := new String'(String_Value (Self, F_Customers_Last));
      Session.Persist (Result);
      return Result;
   end Detach_No_Lookup;
   ...
   begin
      ...
      Free (Tmp.ORM_Author);
      Free (Tmp.ORM_Title);
      Tmp.ORM_Author         := new String'(String_Value (Self, F_Dvds_Author));
      ...
      Tmp.ORM_Title          := new String'(String_Value (Self, F_Dvds_Title));
      Session.Persist (Result);
      return Result;
   end Detach_No_Lookup;
   ...
   function From_Cache
     ...
     return Detached_Customer'Class
   is
      R : Detached_Element_Access;
      D : Detached_Data_Access;
   begin
      Session.From_Cache (Hash_Customer (Id), R, D);
      if R /= null then

         declare
            R2 : Detached_Customer'Class := Detached_Customer'Class (R.all);
         begin
            Set (R2, D);
            return R2;
         end;
   ...
   is
      ...
      D : Detached_Data_Access;
   begin
      Session.From_Cache (Hash_Book (Id), R, D);
      if R /= null then

         declare
            R2 : Detached_Book'Class := Detached_Book'Class (R.all);
         begin
            Set (R2, D);
            return R2;
         end;
   ...
   is
      ...
      D : Detached_Data_Access;
   begin
      Session.From_Cache (Hash_Dvd (Id), R, D);
      if R /= null then

         declare
            R2 : Detached_Dvd'Class := Detached_Dvd'Class (R.all);
         begin
            Set (R2, D);
            return R2;
         end;
   ...
   begin
      if not R.Is_Null then
         ...
         declare
            M : Customers_Managers := All_Customers.Filter
              (Id => Id);
            L : I_Customers.List;
         begin
            M.Select_Related
   ...
   begin
      if not R.Is_Null then
         ...
         declare
            M : Dvds_Managers := All_Dvds.Filter
              (Id => Id);
            L : I_Dvds.List;
         begin
            M.Select_Related
   ...
   overriding function Hash (Self : Detached_Book) return String
   is
      D : constant Book_Data := Book_Data (Self.Get);
   begin
      if D.ORM_Id = -1 then
         return "Book" & System.Address_Image (D.all'Address);
      else
         return Hash_Book (D.ORM_Id);
      end if;
   end Hash;
   ...
   overriding function Hash (Self : Detached_Customer) return String
   is
      D : constant Customer_Data := Customer_Data (Self.Get);
   begin
      if D.ORM_Id = -1 then
         return "Customer" & System.Address_Image (D.all'Address);
      else
         return Hash_Customer (D.ORM_Id);
      end if;
   end Hash;
   ...
   overriding function Hash (Self : Detached_Dvd) return String
   is
      D : constant Dvd_Data := Dvd_Data (Self.Get);
   begin
      if D.ORM_Id = -1 then
         return "Dvd" & System.Address_Image (D.all'Address);
      else
         return Hash_Dvd (D.ORM_Id);
      end if;
   end Hash;
   ...
   function Hash_Book (Id : Integer) return String is
   begin
      return "Book" & Id'Img;
   end Hash_Book;
   ...
   function Hash_Customer (Id : Integer) return String is
   begin
      return "Customer" & Id'Img;
   end Hash_Customer;
   ...
   function Hash_Dvd (Id : Integer) return String is
   begin
      return "Dvd" & Id'Img;
   end Hash_Dvd;
   ...
   begin
      ...
      if Mask (2) then
         if D.ORM_Borrowed_By /= -1 then
            ...
            declare
               ...
               Customer_data (D.ORM_FK_Borrowed_By.Get);
            begin
               if D2.ORM_Id = -1 then
      ...
      if Mask (4) then
         A := A & (DBA.Books.Title = Str_Or_Empty (D.ORM_Title));
      end if;
      if Mask (5) then
         A := A & (DBA.Books.Author = Str_Or_Empty (D.ORM_Author));
      end if;
      if Mask (6) then
   ...
   begin
      if Mask (2) then
         A := A & (DBA.Customers.First = Str_Or_Empty (D.ORM_First));
      end if;
      if Mask (3) then
         A := A & (DBA.Customers.Last = Str_Or_Empty (D.ORM_Last));
      end if;
      if Missing_PK then
         Q := SQL_Insert (A);
      else
   ...
   begin
      ...
      if Mask (4) then
         A := A & (DBA.Dvds.Title = Str_Or_Empty (D.ORM_Title));
      end if;
      if Mask (5) then
         A := A & (DBA.Dvds.Author = Str_Or_Empty (D.ORM_Author));
      end if;
      if Mask (6) then
   ...
   is
      ...
      Data   : constant Book_Data := new Book_DDR;
   begin
      Set (Result, Data);
      return Result;
   end New_Book;
   ...
   is
      ...
      Data   : constant Customer_Data := new Customer_DDR;
   begin
      Set (Result, Data);
      return Result;
   end New_Customer;
   ...
   is
      Result : Detached_Dvd;
      Data   : constant Dvd_Data := new Dvd_DDR;
   begin
      Set (Result, Data);
      return Result;
   end New_Dvd;
