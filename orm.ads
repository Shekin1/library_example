...
package Orm is
   ...
   subtype Books_Managers is I_Books.Manager;
   subtype Books_Stmt is I_Books.ORM_Prepared_Statement;

   All_Books : constant Books_Managers := I_Books.All_Managers;
   ...
   subtype Customers_Managers is I_Customers.Manager;
   subtype Customers_Stmt is I_Customers.ORM_Prepared_Statement;

   All_Customers : constant Customers_Managers := I_Customers.All_Managers;
   ...
   package I_Dvds is new Generic_Managers
     (I_Dvds_Managers, Dvd, Related_Depth, DBA.Dvds,
      Internal_Query_Dvds);
   subtype Dvds_Managers is I_Dvds.Manager;
   subtype Dvds_Stmt is I_Dvds.ORM_Prepared_Statement;

   All_Dvds : constant Dvds_Managers := I_Dvds.All_Managers;
...
private
    ...
    type Book_DDR is new Detached_Data (7) with record
       ORM_Author         : GNAT.Strings.String_Access := null;
       ...
       ORM_Title          : GNAT.Strings.String_Access := null;
    end record;
    type Book_Data is access all Book_DDR;
    
    type Customer_DDR is new Detached_Data (3) with record
       ORM_First    : GNAT.Strings.String_Access := null;
       ORM_Id       : Integer := -1;
       ORM_Last     : GNAT.Strings.String_Access := null;
    end record;
